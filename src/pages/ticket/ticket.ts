...
export class TicketPage {
    ...
    loading:string="";
    datos:boolean =false;
    constructor(public platform:Platform, public navCtrl: NavController, params:NavParams, api:Api, public modal:ModalController, public toast:ToastController,public alert:AlertController)
    {
        this.api = api;
    ...
    getTicket(){
        this.api.getTicket(this.ticket.id).then((data:any)=>{
            ...
            this.toast.create({message: "No se pudo cargar la pagina", duration: 3000,dismissOnPageChange: false,showCloseButton:true}).present();
            this.navCtrl.pop();
        })
    }
    ...
    fechar(fecha)
    {
        return moment(fecha).format("dddd,D MMMM  YYYY, h:mm:ss a") == 'Invalid date' ? 'No Vence' :  moment(fecha).format("dddd,D MMMM  YYYY, h:mm:ss a");
    }
